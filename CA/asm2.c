...
int main()
{
    int L_REVBIT, L_DATOI, L_STRCPY, L_FACT, L_NEXT, L_NEXT2, L_NEXT3, L_END,
...
trap(2);
trap(4);
    BL(L_DATOI);        // Call L_DATOI
    aligncode();
trap(2);
trap(6);
    BRA(L_NEXT2);
L_S1 = addr;
    string("Call function 2: Digital ascii-string to integer (atoi)");
L_S2 = addr;
    string("345");
    aligndata();
...
// Function 1: Reverse the 32-bit bit-order (L_REVBIT)
    aligncode();
L_REVBIT = addr;
    LDI(R1,0);          // R1 = 0
    MV(R2,R0);          // R2 = R0
...
// Function 2: Digital ascii-string to integer (L_DATOI)
    aligncode();
L_DATOI = addr;
    LDI(R1,0);          // R1 = 0
    MV(R2,R0);          // R2 = R0
