...
int main()
{
    int L_REVERSEBYTE, L_TOLOWERCASE, L_NEXT, L_END,
        L0, L1, L2, L3, L4, L5, L6, L7, L8, L_S0, L_S1, L_S2, L_S3, L_S4, L_S5, L_S6;
...
trap(2);                // Display R0-R3
    BL(L_REVERSEBYTE);  // Call L_RVBYTE
    aligncode();
trap(2);                // Display R0-R3
...
L_S0 = addr;
    string("Call function 1: Reverse byte order");
    aligndata();
...
trap(5);                // Display strings at L_S2 and at L_S3
    BL(L_TOLOWERCASE);  // Call L_TOLOWERCASE
    aligncode();
trap(5);                // Display strings at L_S2 and at L_S3
trap(6);                // Display new-line
...
trap(6);                // Display new-line
    BRA(L_END);
L_S1 = addr;
    string("Call function 2: Uppercase to lowercase");
L_S2 = addr;
...
// Function 2: Uppercase to lowercase
    aligncode();
L_TOLOWERCASE = addr;
// You should write a code here
    MV(R2,R0);          // R2 = R0
...
L2 = addr;
    LDB(R4,R2);         // R4 = mem[R2]
    BEQZ(R4,L4);        // if( R4==0 ) goto L4
    ADD3(R5,R4,-64);    // R5 = R4 - 65
    BLEZ(R5,L3);        // if (R5<0) goto L3
    ADD3(R6,R4,-90);    // R6 = R4 - 90
    BGTZ(R6,L3);        // if (R6>0) goto L3
    ADDI(R4,32);        // R4 += 32
    STB(R4,R3);         // mem[R3] = R4
    ADDI(R2,1);         // R2 = R2+1
    ADDI(R3,1);         // R3 = R3+1
